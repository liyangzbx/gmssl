...
class CryptSM2(object):
    ...
    def sm2_get_z(self, ida):
        # print(ida)
        ida_bytes = bytes().fromhex(ida)
        ida_len  = len(ida_bytes)
        # print("ida_len: ", ida_len)
        idbitlen = ida_len * 8
        z_str = self.ecc_table['a'] + self.ecc_table['b'] + self.ecc_table['g'] + self.public_key
        #print(z_str)
        id_s = bytes([int(idbitlen/256), idbitlen & 0xff])
        #print("id_s: ", id_s)
        zbyes = id_s + ida_bytes + bytes().fromhex(z_str)
        #print("zbyes: ", zbyes.hex().upper())
        z=sm3.sm3_hash(func.bytes_to_list(zbyes))
        # print("z hash: ", z.upper())
        return z

    def sm2_get_e(self, ida, msg):
        z = self.sm2_get_z(ida)
        e_bytes = bytes().fromhex(z) + bytes().fromhex(msg)
        # print("e_str: ", e_bytes.hex().upper())
        e = sm3.sm3_hash(func.bytes_to_list(e_bytes))
        # print("e hash: ", e.upper())
        return e
