# -*-coding:utf-8-*-
import binascii
from .func import xor, rotl, get_uint32_be, put_uint32_be, bytes_to_list, list_to_bytes
...
class CryptSM4(object):
    ...
    def crypt_ctr(self, iv, input_data):
        ...
        j = 0
        for j in range(n):
            IV = hex(int(iv.hex(), 16) + j * 1)[2:].upper()
            if len(IV) > 32:
                IV = IV[-32:]
            ...
            length = len(input_data)
            while length > 0:
                tmp_input = self.one_round(self.sk, iv[0:16])
                output_data += xor(input_data[i:i + 16], tmp_input[0:16])
                j += 1
                i += 16
                length -= 16
            return list_to_bytes(output_data)

    def crypt_ofb(self, iv, input_data):
        i = 0
        output_data = []
        iv = bytes_to_list(iv)
        input_data = bytes_to_list(input_data)
